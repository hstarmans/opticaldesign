...
# maximum angle of incidence possible [radians]
utiltmax = np.radians(90-(180-360/facets)/2)
angle = input('Enter angle in degrees \n')
utilt = np.radians(float(angle))  # maximum angle of incidence used [radians]
...
print("The spot radius is "
      + str(round(waist, 2))+" micrometers.")
raileigh = np.pi*waist**2/(wavelength*1000)
print("The rayleigh range is "
      + str(round(raileigh, 3))+" mm.")
...
print("The speed at the edges is "
      + str(round(sdisp.evalf(subs={x: utilt})/1000*speed)) + " m/s.")
# fractional speed
fraction = sdisp.evalf(subs={x: 0})/sdisp.evalf(subs={x: utilt})
print("The speed at the center is "
      + str(round(fraction*100, 2)) + " %" + " of the speed at the edges.")
...
#   Note: cosine has been omitted, will be added back
#         when function f is defined
...
# To calculate the combined RMS we add
# back the functional forms to the coefficients
# They have been obtained from Wyant, page 17, table 2
# The function f defines the wavefront aberration denoted by Wyant as w


def f(theta, rho):
    return (sabr*(rho**4)+astig*(rho**2)*(np.cos(theta)**2)
...
var = (1/np.pi*dblquad(ws, 0, 1, lambda rho: 0, lambda rho: 2*np.pi)[0]
       - 1/(np.pi**2)*(dblquad(w, 0, 1, lambda rho: 0,
                               lambda rho: 2*np.pi)[0]**2))
...
# The Strehl radius is calculated
# with the first three terms of its Taylor series
# Wyant page 39 equation 67
rstrehl = 1-(2*np.pi*lambdarms)**2+(2*np.pi*lambdarms)**4/2
print("The Strehl radius is "
      + str(round(rstrehl, 2)))
