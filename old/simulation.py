...
import rayopt as ro
import warnings
# Rayopt prints out lots of warnings
# this removes them
warnings.filterwarnings("ignore")

# Lens used 12.5mm Dia. x 90mm FL, VIS-NIR, Inked,
# Achromatic Lens from Edmund Optics
...
utilt = np.radians(float(angle))  # tilt angle plate [radians]
...
s.insert(4, ro.elements.Spheroid(distance=10, material='SCHOTT-BK|N-BK7',
                                 diameter=spol*2, angles=[utilt, 0, 0]))
s.insert(5, ro.elements.Spheroid(distance=T/np.cos(utilt),
                                 material='basic/air',
                                 diameter=spol*2,
                                 angles=[utilt, 0, 0]))
...
s.update()
# s.align(s.paraxial.n) # used by jordens for
# astigmatic focus shift, destroys rotation

# astigmatic focus shift , can also be obtained
# from print(q) and looking at table
# print("Astigmatic focus shift "+str(abs(q.waist_position.T[0][-1])
# -abs(q.waist_position.T[1][-1])))+" mm.")
...
# The radau scheme should be able
# to give a good result while using not so many rays
fieldangle = 0
...
print("The Gaussian waist radius is "
      + str(round(q.spot_radius[-1][0]*1000, 2)) + "   micrometers.")
print("The Rayleigh range is "
      + str(q.rayleigh_range[-1][0]) + " mm.")
...
print("The focus point distance from the lens is "
      + str(g.path[-1]-g.path[3])+" mm.")
...
# trick to look for new materials
# lib = ro.Library.one()
# for g in lib.session.query(
#     ro.library.Material).filter(
#     ro.library.Material.name.contains("N-SF5")):
